...
function currencyOptions(currency: string, noLongerSupportedLabel: string) {
	if (CURRENCY_OPTIONS.some((option) => option.value === currency)) {
		return CURRENCY_OPTIONS;
	}

	return [
		{ value: currency, label: `${currency} — ${noLongerSupportedLabel}` },
		...CURRENCY_OPTIONS,
	];
}
...
export function DealSheet({ dealId }: { dealId: string }) {
	...
	return (
		<RecordSheetFrame
			...
			title={deal?.name ?? t.deals.colDeal}
			description={
				deal ? (
					<button
						type="button"
			...
			stats={
				deal ? (
					<DetailSheetStats>
						<DetailSheetStat label={t.deals.colAmount}>
						...
						<DetailSheetStat label={t.deals.colExpectedClose}>
						...
						<DetailSheetStat label={t.deals.inStage}>
							<LocalRelativeTime date={deal.stageChangedAt} />
						</DetailSheetStat>
						<DetailSheetStat label={t.deals.colOwner}>
...
function DealOverview({ deal }: { deal: Deal }) {
	...
	return (
		<DetailSheetBody>
			<DetailSheetSection title={t.deals.colStage}>
				<StageStepper dealId={deal.id} stage={deal.stage} />

				{deal.closedReason ? (
					<DetailSheetProperties>
						<DetailSheetProperty label={t.deals.closed}>
							{deal.closedAt ? (
						...
						<DetailSheetProperty label={t.stages.reason} wide>
							{deal.closedReason}
						</DetailSheetProperty>
					</DetailSheetProperties>
				) : null}
			...
			<DetailSheetSection title={t.common.description}>
				<InlineTextArea
...
function WhereItStands({ deal }: { deal: Deal }) {
	...
	return (
		<DetailSheetSection title={t.deals.whereItStands}>
			<DetailSheetProperties>
				<DetailSheetProperty label={t.deals.opened}>
					<LocalDateTime date={deal.createdAt} options={DATE_OPTIONS} />
				</DetailSheetProperty>

				<DetailSheetProperty label={t.deals.inStageSince}>
				...
				<DetailSheetProperty label={t.deals.onIt} wide>
					{deal.contacts.length === 0 ? (
						<span className="text-muted-foreground">
							{t.deals.noContactsOnDeal}
						</span>
					) : (
...
function DealContacts({
	...
	const contactColumns = [
		...
		{ id: "remove", srLabel: t.settings.remove, width: "w-10" },
	];
		...
		return (
			<>
				...
				{adding ? null : (
					<DetailSheetEmpty
						icon={UserMultiple}
						title={t.deals.noContactsOnDeal}
						description={t.recordSheet.noContactsOnDealDesc}
						action={
				...
				{deal.contacts.map((contact) => (
					...
					>
						...
						<TableCell className="truncate px-1 py-2.5">
							<InlineTextCell
								label={`${t.deals.roleOnDeal} for ${contactName(contact)}`}
								value={contact.role}
								placeholder={t.deals.roleOnDeal}
								saving={
									setRole.isPending &&
